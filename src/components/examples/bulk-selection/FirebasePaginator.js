/* eslint-disable */
export function FirebasePaginator(ref, defaults) {
  var paginator = this;
  ...
  if (!isFinite) {
    ...
    this.goToPage = function goToPage(pageNumber, newPageSize) {
      newPageSize = typeof newPageSize === 'undefined' ? 10 : newPageSize;
      console.log('paginator goToPage: ', paginator);
      console.log('paginator goToPage pageNumber: ', pageNumber);
      console.log('paginator goToPage newPageSize: ', newPageSize);

      /*
      pageNumber = Math.min(
        paginator.pageCount,
        Math.max(1, parseInt(pageNumber))
      );*/

      //pageNumber = 2;

      if (Object.keys(paginator.pages || {}).length) {
        console.log('paginator 1');
        // Null check for empty collections
        paginator.page = paginator.pages[pageNumber];
        paginator.pageNumber = pageNumber;
        paginator.isLastPage =
          pageNumber === Object.keys(paginator.pages).length;
        paginator.ref = ref
          .orderByKey()
          .limitToLast(newPageSize)
          .endAt(paginator.page.endKey);
      } else {
        console.log('paginator 2 newPageSize: ', newPageSize);
        paginator.ref = ref.orderByKey().limitToLast(newPageSize);
      }

      return paginator.ref.once('value').then(function(snap) {
